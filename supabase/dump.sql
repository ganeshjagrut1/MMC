-- =============================================================
--  MAULI MEDICAL COLLEGE — COMPLETE SQL DUMP (run once)
--  Supabase Dashboard -> SQL Editor -> New query -> paste -> Run
--
--  This single file sets up EVERYTHING:
--    1. Tables
--    2. Row Level Security policies
--    3. Seed content (21 departments + sample news)
--    4. Admin login account
--
--  Safe to re-run (idempotent).
--
--  After running, sign in at  /admin  with:
...
insert into public.news (slug, title, excerpt, content, category, cover_image_url, published, published_at) values
  ('welcome-mauli',
   'Welcome to Mauli Medical College',
   'A premier institution for medical education, healthcare and research.',
   'Mauli Medical College, Hospital & Research Center welcomes students, patients and the community to a campus dedicated to excellence in medical sciences.',
   'Announcement', '/images/news1.jpg', true, now()),
...
-- ---------- 4. Admin login account ----------
-- Email:    collegemaulimedical@gmail.com
-- Password: Mauli@Admin2026   (change the value below if you wish)
...
begin
  ...
  if v_uid is null then
    ...
    -- User already exists: reset password + confirm email
    update auth.users
  ...
  -- Ensure the email identity exists (required for password login)
  if not exists (
  ...
  raise notice 'Admin ready: % (id %)', v_email, v_uid;
end $$;
